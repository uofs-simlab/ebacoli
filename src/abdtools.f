...
C     COPY TOP BLOCK INTO DENSE
      DO J = 1, NOVRLP
C     JJ IS POINTER TO FIRST ELEMENT OF COLUMN I IN TOPBLK
         JJ = 1+(J-1)*NRWTOP
C     DD IS POINTER TO FIRST ELEMENT OF COLUMN I IN DENSE
         DD = 1+(J-1)*N
         CALL DCOPY(NRWTOP,TOPBLK(JJ),1,DENSE(DD),1)
      END DO
C
C     COPY MIDDLE BLOCKS INTO DENSE
      DO I = 1, NBLOKS
...
C     COPY TOP BLOCK INTO DENSE MATRIX
      ...
      DJ = N - NOVRLP + 1
      DO J = 1, NOVRLP
C     JJ IS THE POINTER TO THE FIRST ELEMENT OF COLUMN J IN BOTBLK
         JJ = 1 + (J-1)*NRWBOT
C     DD IS POINTER INTO THE FIRST ELEMENT OF COLUMN J OF THE BOTTOM
C     BLOCK IN DENSE
         DD = DI + (DJ-1+J-1)*N
         CALL DCOPY(NRWBOT,BOTBLK(JJ),1,DENSE(DD),1)
      END DO
C
      IFLAG = 0
      RETURN
...
C      .
C      .
C      DENSE(I): COLUMN MAJOR ORDERED ENTRIES OF DENSE, 1 PER LINE
C      .
C      .
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
C***************************************************************
C
C  THIS PROGRAM  COMPUTES THE CONDITION NUMBER OF AN ALMOST BLOCK
C  DIAGONAL MATRIX OF THE FORM:
C
C               TOPBLK
C               ARRAY(1)
...
C       *** ON RETURN  ...
C
C               EST    - ESTIMATED CONDITION NUMBER.
...
C
C       ABDNRM(NBLOKS,NTOP,NBOT,NOVRLP,
C    *         NRWBLK,NCLBLK,TOP,A,BOT)
C              COMPUTES THE NORM OF THE ABD MATRIX TOP, A, BOT.
C
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
C
C       DOUBLE PRECISION TOPBLK,ARRAY,BOTBLK
     ...
     *          BOTBLK(NRWBOT,*),EST,V(*),WORK(*)
        DOUBLE PRECISION ABDNRM,NORM
        INTEGER KASE,ISOLVE,NOUT
        NOUT = 6
C
C       FIRST, COMPUTE THE NORM OF THE MATRIX ITSELF:
C
        NORM = ABDNRM(NBLOKS,NRWTOP,NRWBOT,NOVRLP,NRWBLK,NCLBLK,
     *                TOPBLK,ARRAY,BOTBLK)
C
