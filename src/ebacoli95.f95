...
! 1. The eBACOLI solver routines in ebacoli.f
! 2. The supporting routines collected in ebacoli-aux.f.
...
module ebacoli95_mod
! This module provides a convenient Fortran 95 wrapper for eBACOLI-3,
! the current version of ebacoli.f, written in Fortran 77. In this
! source code eBACOLI-3 will be referred to simply as eBACOLI.
    ...
    !                        eBACOLI. This is that maximum.
    !                        Set by ebacoli95_init.
        integer           :: kcol
    !                        eBACOLI uses collocation methods. kcol is
    !                        the number of collocation points per mesh
    !                        subinterval and determines the order of
    !                        piecewise polynomials used.
    !                        Set by ebacoli95_init.
        integer           :: estimator
    !                        eBACOLI has two options for spatial error
    ...
    !                        of length 1 or length npde. eBACOLI
    ...
    !                        to eBACOLI (ebacoli.f) documentation.
    !                        Set by ebacoli95_init and ebacoli95.
        real(dp), pointer :: y(:)
    !                        eBACOLI approximates solutions using
    ...
    !                        information, use ebacoli95_vals or an
    !                        alternative described along with the
    !                        ebacoli95_splines type (see below).
    !                        Set by ebacoli95.
        real(dp), pointer :: rpar(:)
    !                        rpar is a large work array for eBACOLI.
    !                        Care should be taken when accessing or
    !                        modifying this memory.
    !                        Set by ebacoli95_init and ebacoli95.
        integer,  pointer :: ipar(:)
    !                        ipar is a work array for eBACOLI.
    !                        Care should be taken when accessing or
    !                        modifying this memory.
    !                        Set by ebacoli95_init and ebacoli95.
    !
    ! miscellaneous counters and values (output by ebacoli95)
    ...
    !           previous call to ebacoli95 in the case of continuation
    !           calls.
    ! tout:   Next specific output time. ebacoli95 will return with
    !           sol%t0 <= tout so that solution information may be
    !           obtained (unless an error condition arose).
    !           ebacoli95 may also successfully return due to other
    ...
    !             ebacoli95_sol documentation for detailed comments.
    ...
    !             have been printed.
    !             See eBACOLI documentation for details.

    public :: ebacoli95_vals
    ...
    !
    !   subroutine ebacoli95_vals(sol, xout, uout, nderiv)
    !
    ! Required Input:
    ! sol:    Solution data returned by a successful call to ebacoli95.
    ...
    public :: ebacoli95_sol_teardown
    ! Deallocate and nullify pointers inside sol.
    !
    !   subroutine ebacoli95_sol_teardown(sol)

    public :: ebacoli95_output_Bsplines

! eBACOLI represents the approximate solution that it computes as a
! piecewise polynomial (a B-spline). While this wrapper and eBACOLI both
...
! programs.
!
! The ebacoli95_splines type contains only the information from the
! ebacoli95_sol type which pertains to the B-spline at a specific time.
! This time is the t0 field of the ebacoli95_sol type, and the t field
! of the ebacoli95_splines type. It is hoped that this provides the user
! with more flexibility in manipulating solutions returned by ebacoli95.
!
! For an example making use of the ebacoli95_sol_to_splines routine,
...
! Python script is included within the comments of driver95-curve.f95.
    ...
    type, public :: ebacoli95_splines
    ! This structured type is provided for conveniently exporting
    ! B-spline information produced by eBACOLI.
        integer           :: npde
    ...
    !                        npde is the number of B-splines contained
    !                        within this type. There is a separate
    !                        spline for each solution component in u.
        integer           :: p
    ...
    !                        eBACOLI uses spatial mesh points as knots,
    !                        so these are obtained by repeating values
    !                        of sol%x.
    end type

    public :: ebacoli95_sol_to_splines
    ...
    !
    !   subroutine ebacoli95_sol_to_splines(sol, splines, iflag)
    !
    ! Input:
    ! sol:     Solution data returned by a successful call to ebacoli95.
    !
    ...
    public :: ebacoli95_splines_teardown
    ! Deallocate and nullify pointers inside splines.
    !
    !   subroutine ebacoli95_splines_teardown(splines)
    ...
    contains
       subroutine ebacoli95_init(sol, npde_sub, x, tstart, atol, rtol, &
            ...
            if (size(x) == 2) then
                ...
                sol%x(1:dflt_nint+1) = (/x(1),(x(1)+i*(x(2)-x(1))/(dflt_nint), &
                        i=1,dflt_nint)/)
                sol%nint = dflt_nint
            else
                sol%x(1:size(x)) = x
         ...
         99 continue
            ...
            select case (sol%idid)
                case(-1000)
                    write(eunit,*) 'allocation error in ebacoli95_init'
                case(-1001)
...
!-----------------------------------------------------------------------
        ...
        subroutine ebacoli95(sol, tout, f, bndxa, bndxb, uinit, &
            ...
            if (sol%idid < 0) then
                write(eunit,*) 'eBACOLI previously returned with an error'
            ...
            if (iflag == 1) then
                ...
                write(eunit,*) 'ERROR: array lengths too short to call ebacoli'
                return
            end if

            ! eBACOLI allows for the finite difference approximation
            ...
            if (present(tstop)) then
                ! eBACOLI complains if tstop < tout
                outt = min(tout, tstop)
                sol%mflag(3) = 1
            else
                outt = tout
            ...
            ! determined user can, alternatively, read eBACOLI source code
...
!-----------------------------------------------------------------------
        ...
        subroutine ebacoli95_sol_to_splines(sol, splines, iflag)
            type(ebacoli95_sol)     :: sol
            type(ebacoli95_splines) :: splines
         ...
         99 continue
            ...
            select case (iflag)
                ...
                case(-2)
                    call ebacoli95_splines_teardown(splines)
                    write(eunit,*) 'allocation error in ebacoli95_sol_to_splines'
                case default
                    write(eunit,*) 'error of unknown type'
            end select
        end subroutine
...
!-----------------------------------------------------------------------
        ...
        subroutine ebacoli95_sol_trim(sol)
            ! Currently unused.
            type(ebacoli95_sol) :: sol
...
!-----------------------------------------------------------------------
        ...
        subroutine check_array_sizes(sol, iflag)
            ...
            ! iflag = 2 : array lengths below min (may segfault in ebacoli)
...
!-----------------------------------------------------------------------
        ...
        subroutine ebacoli95_output_Bsplines(sol, step, time)
          ! parameters
          implicit none
          type(ebacoli95_sol)          :: sol
          ...
          integer                      :: ier
          type(ebacoli95_splines)      :: splines
          character(len=32)            :: fname, step_str

          call ebacoli95_sol_to_splines(sol, splines, ier)
        ...
        end subroutine ebacoli95_output_Bsplines
