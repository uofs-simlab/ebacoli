...
C     Hydraulic conductivity K(psi)
      double precision ss,phi,thetas,thetar,alpha,n,m,ks
      common /richards/ ss,phi,thetas,thetar,alpha,n,m,ks
      hydcon = ks * (0.1D1 - (alpha * dabs(psi)) ** (n - 0.1D1) * (0.1D
     $     1 + (alpha * dabs(psi)) ** n) ** (-m)) ** 2 / (0.1D1 + (alpha
     $     * dabs (psi)) ** n) ** (m / 0.2D1)
      return
      end
...
C     Derivative of hydraulic conductivity K'(psi)
      double precision ss,phi,thetas,thetar,alpha,n,m,ks
      common /richards/ ss,phi,thetas,thetar,alpha,n,m,ks
      dhydco = 0.2D1 * ks * (0.1D1 - (alpha * dabs(psi)) ** (n - 0.1D1)
     $     * (0.1D1 + (alpha * dabs(psi)) ** n) ** (-m)) / (0.1D1 +
     $     (alpha * dabs(psi)) ** n) ** (m / 0.2D1) * (-(alpha *
     $     dabs(psi)) ** (n - 0.1D 1) * (n - 0.1D1) * dabs(psi) / psi /
     $     dabs(psi) * (0.1D1 + (alpha * a bs(psi)) ** n) ** (-m) +
     $     (alpha * dabs(psi)) ** (n - 0.1D1) * (0.1D 1 + (alpha *
     $     dabs(psi)) ** n) ** (-m) * m * (alpha * dabs(psi)) ** n * n *
     $     dabs(psi) / psi / dabs(psi) / (0.1D1 + (alpha * dabs(psi)) **
     $     n)) - ks * (0.1D1 - (alpha * dabs(psi)) ** (n - 0.1D1) *
     $     (0.1D1 + ( alpha * dabs(psi)) ** n) ** (-m)) ** 2 / (0.1D1 +
     $     (alpha * dabs(psi)) ** n) ** (m / 0.2D1) * m * (alpha *
     $     dabs(psi)) ** n * n * dabs(psi) / psi / dabs(psi) / (0.1D1 +
     $     (alpha * dabs(psi)) ** n) / 0.2D1
      return
      end
