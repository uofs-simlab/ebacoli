...
c-----------------------------------------------------------------------
c parameters
      double precision ss,phi
      common /vanderborght/ ss,phi
c-----------------------------------------------------------------------
c functions
      double precision volcon, dvolco, ddvolc,
     &     hydcon, dhydco, ddhydc, qs
      doubleprecision thetar,thetas,alpha,exponent,ks
c
c-----------------------------------------------------------------------
c Local variables
...
C     Source/sink term
      ...
      doubleprecision function hydcon (psi,x)
      doubleprecision psi,x
      doubleprecision m,n
C     Hydraulic conductivity K(psi)
      ...
      n = exponent(x)
      m = 1.d0-1.d0/n
      hydcon = ks(x) * (0.1D1 - (alpha(x) * dabs(psi)) ** (n - 0.1D1) *
     $     (0.1D1 + (alpha(x) * dabs(psi)) ** n) ** (-m)) ** 2 / (0.1D1
     $     + (alpha(x)* dabs (psi)) ** n) ** (m / 0.2D1)
      return
      end
      doubleprecision function dhydco (psi,x)
      doubleprecision psi,x
      doubleprecision m,n
C     Derivative of hydraulic conductivity K'(psi)
      ...
      n = exponent(x)
      m = 1.d0-1.d0/n
      dhydco = 0.2D1 * ks * (0.1D1 - (alpha(x) * dabs(psi)) ** (n -
     $     0.1D1)* (0.1D1 + (alpha(x) * dabs(psi)) ** n) ** (-m)) /
     $     (0.1D1 +(alpha(x) * dabs(psi)) ** n) ** (m / 0.2D1) * (
     $     -(alpha(x) *dabs(psi)) ** (n - 0.1D 1) * (n - 0.1D1) *
     $     dabs(psi) / psi /dabs(psi) * (0.1D1 + (alpha(x) * a bs(psi))
     $     ** n) ** (-m) +(alpha(x) * dabs(psi)) ** (n - 0.1D1) * (0.1D
     $     1 + (alpha(x) *dabs(psi)) ** n) ** (-m) * m * (alpha(x) *
     $     dabs(psi)) ** n * n *dabs(psi) / psi / dabs(psi) / (0.1D1 +
     $     (alpha(x) * dabs(psi)) **n)) - ks * (0.1D1 - (alpha(x) *
     $     dabs(psi)) ** (n - 0.1D1) *(0.1D1 + ( alpha(x) * dabs(psi))
     $     ** n) ** (-m)) ** 2 / (0.1D1 +(alpha(x) * dabs(psi)) ** n) **
     $     (m / 0.2D1) * m * (alpha(x) *dabs(psi)) ** n * n * dabs(psi)
     $     / psi / dabs(psi) / (0.1D1 +(alpha(x) * dabs(psi)) ** n) /
     $     0.2D1
      return
      end
      doubleprecision function ddhydc (psi,x)
        doubleprecision psi,x
        doubleprecision m,n
        doubleprecision s0
        doubleprecision s1
...
C     2nd derivative of hydraulic conductivity K''(psi)
     ...
     $       * s5 6 + 0.2D1 * ks * s155 * (-(n - 0.1D1) ** 2 * s72 * s73
     $       + (-n + 0.1 D1) * s79 * s80 + (n - 0.1D1) * s86 * s84 **
     $       s85 + (0.2D1 * n - 0. 2D1) * m * n * s97 / s94 - m ** 2 * n
     $       ** 2 * s109 / s106 ** 2 + m * n ** 2 * s120 / s117 + m * n
     $       * s131 / s128 - m * n * s142 / s139 - m * n ** 2 * s154 /
     $       s151 ** 2) + ks * m ** 2 * n ** 2 * s172 / s169 ** 2 /
     $       0.4D1 - ks * m * n ** 2 * s188 / s185 / 0.2D1 - ks * m * n
     $       * s204 / s201 / 0.2D1 + ks * m * n * s220 / s217 / 0.2D1 +
     $       ks * m * n ** 2 * s237 / s234 ** 2 / 0.2D1
      return
      end
      doubleprecision function thetar(x)
      doubleprecision x
C     Space-dependent thetar value
      if (x .lt. 0.5d0) then
         thetar = 0.080d0
      else
         thetar = 0.045d0
      ...
      doubleprecision function thetas(x)
      doubleprecision x
C     Space-dependent thetas value
      if (x .lt. 0.5d0) then
         thetas = 0.430d0
      else
         thetas = 0.430d0
      ...
      doubleprecision function alpha(x)
      doubleprecision x
C     Space-dependent alpha value
      if (x .lt. 0.5d0) then
         alpha = 4.d0
      else
         alpha = 15.d0
      ...
      doubleprecision function exponent(x)
      doubleprecision x
C     Space-dependent exponent value
      if (x .lt. 0.5d0) then
         exponent = 1.6d0
      else
         exponent = 3.0d0
      ...
      doubleprecision function ks(x)
      doubleprecision x
C     Space-dependent ks value
      if (x .lt. 0.5d0) then
         ks = 5.79d-6
      else
         ks = 1.16d-4
      ...
      doubleprecision function volcon (psi,x)
      doubleprecision psi,x
C     Volumetric water content theta(psi)
     ...
     $     * m + thetar
      ...
      doubleprecision function dvolco (psi,x)
      doubleprecision psi,x
C     Derivative of volumetric water content theta'(psi)
     ...
     $     dabs(psi) / (0.1D1 + (alpha * dabs(psi)) ** n)
      ...
      doubleprecision function ddvolc (psi,x)
      doubleprecision psi,x
