...
c
c     u_t = x u_xx + t x u_x + (1 + t x) e^{-tx} v
c     v_t = -0.5 e^{tx} u - 0.5 \sqrt{1-v^2}
c
c     1 <= x <= 2,   0 <= t <= 1
c
c     BC:
c                u(1,t) - e^{-t} \sin{1+t} = 0
c     u_x(2,t) + t u(2,t) - e^{-2t} v(2,t) = 0
c
c     IC:
c     u(x,0) = \sin{x}
c     v(x,0) = \cos{x}
...
c     The parabolic equation
      FVAL(1)= X*UXX(1) + T*X*UX(1) + (1+T*X)*DEXP(-T*X)*U(2)

c     The ODE
      FVAL(2) = -0.5D0*(DEXP(T*X)*U(1) + DSQRT(1-U(2)*U(2)))
C
      RETURN
      END
...
C
      DFDU(1,1) = 0.D0
      DFDU(1,2) = (1+T*X)*DEXP(-T*X)

      DFDU(2,1) = -0.5D0*DEXP(T*X)
      DFDU(2,2) = -0.5D0*DSQRT(1-U(2)*U(2))*U(2)
c
      DFDUX(1,1) = T*X
      DFDUX(1,2) = 0.D0
...
c Loop indices:
        integer                 i
C-----------------------------------------------------------------------
      BVAL(1) = UX(1) + T*U(1) - DEXP(-2.D0*T)*U(2)
C
      RETURN
      END
